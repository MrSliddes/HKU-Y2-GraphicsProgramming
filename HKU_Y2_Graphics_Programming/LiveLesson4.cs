	...
	class LiveLesson4 : Lesson
	{
		...
		private void GeneratePlane(float gridSize = 8.0f, float height = 128f) {
            ...
            for(int y = 0; y < heightmap.Height; y++)
            {
                ...
                {
					int index = y * heightmap.Width + x; // unieke pixel id
					...
					vertices[index] = new Vert(new Vector3(gridSize * x, pixels[index].R / 255f * height, gridSize * y),
						Vector3.Up, Vector3.Up, Vector3.Up,
						new Vector2(x / (float)heightmap.Width, y / (float)heightmap.Height));
					// if not at edge
					if(y < heightmap.Height - 2 && x < heightmap.Width -2)
                    {
						// add incices for 2 two triangles (bottom-right)
		...
		public override void Draw(GameTime gameTime, GraphicsDeviceManager graphics, SpriteBatch spriteBatch) {
			...
			Matrix Projection = Matrix.CreatePerspectiveFieldOfView((MathF.PI / 180f) * 65f, device.Viewport.AspectRatio, 0.01f, 5000f);
